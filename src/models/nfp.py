...
class NfpPredictor(Predictor):
    ...
    def __init__(self, radius=4, fplength=64):
        ...
        self.model_params = dict(fp_length=fplength,    # Usually neural fps need far fewer dimensions than morgan.
                    ...
                    h1_size=128,     # Size of hidden layer of network on top of fps.
                    h2_size=128,     # Size of hidden layer of network on top of fps.
                    L2_reg=np.exp(-2))
       
        self.train_params = dict(num_iters=500,
                    batch_size=64,
                    init_scale=np.exp(-4),
                    step_size=np.exp(-6))
    ...
    def fit(self, smiles_list, logS_list, seed=0):
        ...
        net_arch_params = dict(
            layer_sizes = [self.model_params['fp_length'], self.model_params['h1_size'], self.model_params['h2_size']],  # One hidden layer
            normalize=True, L2_reg = self.model_params['L2_reg'], nll_func = rmse)

        loss_fun, pred_fun, conv_parser = build_conv_deep_net(conv_arch_params, net_arch_params, self.model_params['L2_reg'])
        
        num_weights = len(conv_parser)
#        predict_func, trained_weights, conv_training_curve = self._train_nn(pred_fun, loss_fun, num_weights, train_smiles, train_logS, 
#                                                validation_smiles=None, validation_raw_targets=None)
        
#        test_predictions = predict_func(test_inputs)

    
#    def _train_nn(self, pred_fun, loss_fun, num_weights, train_smiles, train_raw_targets, seed=0, validation_smiles=None, validation_raw_targets=None):
...
#                    print("Validation RMSE", iter, ":", rmse(validation_preds, validation_raw_targets), end=' ')
        ...
        grad_fun_with_data = build_batched_grad(grad_fun, self.train_params['batch_size'],
                                            train_smiles, train_logS_norm)

        # Optimize weights.
        trained_weights = adam(grad_fun_with_data, init_weights, #callback=callback,
                           num_iters=self.train_params['num_iters'], step_size=self.train_params['step_size'])

         
        self.model = (undo_norm, trained_weights, pred_fun) 
#        self._undo_norm = undo_norm
